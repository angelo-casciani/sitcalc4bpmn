/*
  Complete job application BPMN with exogenous actions to control the process flow.
*/
...
poss(check_application_for_interview(start, ID), and(and(done(assign_contact_partner(end,  ID)), done(verify_prerequisites(end,  ID))), application(ID))).
...
% proc(running(check_validity(start, N)),
%   and(
%     done(check_validity(start, N)),
%     neg(done(check_validity(end, N, _))))) :- !.
...
proc(applicant(ID),
  ...
  ?(done(prepare_application(end,  ID))),
    ...
    application_sent(ID),
      ...
      ndet(
        [?(done(contract_sent(ID))),
          ...
          ?(done(sign_contract(end,  ID))),
          [signed_contract_sent(ID),
                  communicate_recruitment(start, ID),
                  ?(done(communicate_recruitment(end,  ID))),
                  application_finalised(ID)]
        ],
...
proc(company(ID),
      ...
      [
        ...
        check_validity(start, ID),
        ?(some(result, done(check_validity(end,  ID, result)))),
        if(documents_ok(ID),
          [
                  ...
                  [
                    ...
                    if(approval(ID),
                      ...
                      [
                        ...
                        ?(done(produce_letter_of_refusal(end,  ID))),
                        [
